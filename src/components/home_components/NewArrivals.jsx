      ...
      <div className="flex flex-col gap-2">
        <div className="relative w-full">
          <button className="rounded-full p-2 bg-[var(--text-secondary-light)] shadow-[0px_0px_10px_-5px_black] absolute top-2 right-2">
            <FaRegHeart color="var(--main-primary)" size={22} />
          </button>
          <img className="w-full object-cover" src={`designs/${images}`} alt={`${name} image`} />
        </div>
        <p className="text-lg text-[var(--text-primary)] text-left font-medium">{name}</p>
        <p className="text-base text-[var(--main-secondary)] text-left font-medium">{description}</p>
        <p className="text-xl text-[var(--main-primary)] text-left font-semibold">#{price}</p>
      </div>
    </>
  );
};
...
const NewArrivals = () => {
  ...
  const newArrivalsMockDataset = [
    ...
    },
    {
      ...
      price: 65000,
      images: "Nigerian Coral Bead Jewelry Set_ African Wedding Necklace, Bracelet & Earrings 1.png",
    },
    {
    ...
    },
    {
      ...
      price: 320000,
      images: "Hand-Carved Wooden Sun Wall Sculpture_ Gold & Black Abstract Art (1) 1.png",
    },
    {
      ...
      price: 7500,
      images: "Afro-Chic Ankara Adire Kimono_ Handgemachte Afrikanische Print Boho Jacke 1.png",
    },
  ];

  return (
    <>
      <section className={`${homeSectionPadding} bg-[var(--text-secondary-light)] w-full min-w-screen`}>
        <h2 className="w-full text-center text-[var(--text-primary)] font-normal text-2xl mb-3">New <span className="text-[var(--main-primary)]">Arrivals</span></h2>
        <div className={`grid ${currentSize < 480 ? "grid-cols-1" : "grid-cols-2"} md:grid-cols-3 xl:grid-cols-4 gap-3 justify-start`}>
          {newArrivalsMockDataset.map(
            (data, index) => {
